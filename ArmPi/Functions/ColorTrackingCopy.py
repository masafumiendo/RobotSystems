...
# Arguments are the list of contours to compare
...
# The angle of closure when the gripper is clamped
servo1 = 500
...
def move():
    ...
    # Coordinates (x, y, z) of different color wood fast placement
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # When the object is first detected
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) # No runtime parameter, adaptive runtime
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                if track: # If it is the tracking phase
                    ...
                    time.sleep(0.02)                    
                    track = False
                if start_pick_up: # If the object does not move for a while, start clamping
                    action_finish = False
                    if not __isRunning: # Stop and exit flag bit detection
                        continue
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1, 500)  # Clamp closure
                    time.sleep(1)
                    
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(2, 500, 500)
                    AK.setPitchRangeMoving((world_X, world_Y, 12), -90, -90, 0, 1000)  # Robotic arm lift
                    time.sleep(1)
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Claws open and drop objects
                    time.sleep(0.8)
...
def run(img):
    ...
    if not start_pick_up:
        ...
        if area_max > 2500:  # Have found the maximum area
            ...
            cv2.putText(img, '(' + str(world_x) + ',' + str(world_y) + ')', (min(box[0, 0], box[2, 0]), box[2, 1] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, range_rgb[detect_color], 1) #Plot the center point
            distance = math.sqrt(pow(world_x - last_x, 2) + pow(world_y - last_y, 2)) #Compare last coordinates to determine if it is moving
